 ...
 * those pointers.
 */
void* van_emde_boas_tree::Node::operator new(size_t size, size_t numPointers) {
 ...
 * name it.
 */
void van_emde_boas_tree::Node::operator delete(void* memory, size_t) {
...
van_emde_boas_tree::const_iterator::const_iterator() {
  mCurr = van_emde_boas_tree::kNil;
...
van_emde_boas_tree::const_iterator::const_iterator(size_t valueOrSentinel,
                                                const van_emde_boas_tree* owner) {
  mCurr = valueOrSentinel;
  mOwner = owner;
}

/* Equality checks for equality of the underlying value and tree. */
bool van_emde_boas_tree::const_iterator::operator== (const const_iterator& rhs) const {
 ...
 * place.
 */
const unsigned short van_emde_boas_tree::const_iterator::operator* () const {
 ...
 * current value.
 */
van_emde_boas_tree::const_iterator& 
van_emde_boas_tree::const_iterator::operator ++() {
...
const van_emde_boas_tree::const_iterator
van_emde_boas_tree::const_iterator::operator++ (int) {
...
van_emde_boas_tree::const_iterator::operator --() {
   ...
   * tree.
   */
  if (mCurr == van_emde_boas_tree::kNil) {
    mCurr = van_emde_boas_tree::treeMax(mOwner->mRoot, kShortBits);
  } 
...
/**** Implementation of van_emde_boas_tree interface. */

/* Constructor recursively constructs all of the tree structure. */
van_emde_boas_tree::van_emde_boas_tree() {
...
van_emde_boas_tree::van_emde_boas_tree(const van_emde_boas_tree& other) {
...
van_emde_boas_tree& van_emde_boas_tree::operator= (const van_emde_boas_tree& other) {
  van_emde_boas_tree copy = other;
  swap(copy);
  return *this;
}

/* begin returns a const_iterator to the smallest value in the tree. */
van_emde_boas_tree::const_iterator van_emde_boas_tree::begin() const {
  return const_iterator(treeMin(mRoot, kShortBits), this);
}

/* end returns a const_iterator to the sentinel value. */
van_emde_boas_tree::const_iterator van_emde_boas_tree::end() const {
  return const_iterator(kNil, this);
}

/* Reverse begin and end functions just wrap up end and begin, respectively. */
van_emde_boas_tree::const_reverse_iterator van_emde_boas_tree::rbegin() const {
  return const_reverse_iterator(end());
}
van_emde_boas_tree::const_reverse_iterator van_emde_boas_tree::rend() const {
  return const_reverse_iterator(begin());
}

/* size hands back the cached size. */
size_t van_emde_boas_tree::size() const {
  return mSize;
}

/* empty reports whether the size is zero. */
bool van_emde_boas_tree::empty() const {
  return size() == 0;
}
 ...
 */
van_emde_boas_tree::const_iterator van_emde_boas_tree::find(unsigned short value) const {
 ...
 */
std::pair<van_emde_boas_tree::const_iterator, bool>
van_emde_boas_tree::insert(unsigned short value) {
...
bool van_emde_boas_tree::erase(unsigned short value) {
 ...
 */
void* van_emde_boas_tree::recCreateTree(size_t numBits) {
 ...
 * and freeing them.
 */
void van_emde_boas_tree::recDeleteTree(void* root, size_t numBits) {
 ...
 */
bool van_emde_boas_tree::recFindElement(unsigned short value, void* root, 
                                     size_t numBits) {
 ...
 */
bool van_emde_boas_tree::recInsertElement(unsigned short value, void* root, 
                                       size_t numBits) {
 ...
 */
size_t van_emde_boas_tree::treeMax(void* root, size_t numBits) {
...
size_t van_emde_boas_tree::treeMin(void* root, size_t numBits) {
  /* If the tree is a bitvector, march down the bits checking where the
   * smallest is.
   */
  if (numBits <= kBitvectorSize) {
 ...
 * of tree.
 */
bool van_emde_boas_tree::isTreeEmpty(void* root, size_t numBits) {
 ...
 * deleting from.
 */
bool van_emde_boas_tree::recEraseElement(unsigned short value, void* root,
                                      size_t numBits) {
...
size_t van_emde_boas_tree::recSuccessor(unsigned short value, void* root,
                                     size_t numBits) {
...
size_t van_emde_boas_tree::recPredecessor(unsigned short value, void* root,
                                       size_t numBits) {
  /* If the tree is a bitvector, our search for a successor just involves
   * scanning the bits.
   */
...
void* van_emde_boas_tree::recCloneTree(void* root, size_t numBits) {
